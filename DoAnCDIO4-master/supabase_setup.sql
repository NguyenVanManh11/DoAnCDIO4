-- Supabase / PostgreSQL Setup Script for Coffee Shop Management System
...
-- 2. Bảng Hạng Thành Viên (hangthanhvien)
...
-- 8. Bảng Size Sản Phẩm (sanphamsize)
...
-- 10. Bảng Sản Phẩm Topping (sanphamtopping)
...
-- Seed Voucher
INSERT INTO voucher (MaVoucher, TenVoucher, LoaiGiamGia, GiaTriGiam, GiaTriGiamToiDa, DonHangToiThieu, SoLuong, SoLanDungToiDa, TrangThai) VALUES
('GIAM10K', 'Giảm trực tiếp 10K', 'TienMat', 10000.00, 10000.00, 20000.00, 1000, 1, 'HoatDong')
ON CONFLICT (MaVoucher) DO NOTHING;

-- 13. Bảng Đơn Hàng (donhang)
...
-- 14. Bảng Chi Tiết Đơn Hàng (chitietdonhang)
CREATE TABLE IF NOT EXISTS chitietdonhang (
    ...
    GhiChu TEXT,
    TrangThai VARCHAR(20) DEFAULT 'ChoPhaChe' -- 'ChoPhaChe', 'DangPhaChe', 'HoanThanh', 'DaGiao'
);

-- 15. Bảng Chi Tiết Topping (chitiettopping)
...
CREATE TABLE IF NOT EXISTS datban (
    ...
    SoNguoi INT,
    GhiChu TEXT,
    TrangThai VARCHAR(20) DEFAULT 'ChoXacNhan', -- 'ChoXacNhan', 'DaXacNhan', 'DaHuy', 'HoanThanh'
    NgayTao TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- 17. Bảng Đánh Giá (danhgia)
...
-- 18. Bảng Nguyên Liệu (nguyenlieu)
...
-- 19. Bảng Công Thức Pha Chế (congthucphache)
...
-- 22. Bảng Phân Công Ca Làm (phancongcalam)
...
-- Seed Người Dùng Demo
-- Mật khẩu demo là '123' cho khách và 'admin123' cho admin
...
-- Seed Đơn Hàng mẫu cho Khách Hàng 1
...
-- Đơn DH1002: Cà phê đen truyền thống (SanPhamID 1, Size S = 25000) x 2, Croissant (SanPhamID 4, Size S = 35000) x 1. Tổng cộng: 85,000 đ
-- Đơn DH0981: Bạc xỉu đá (SanPhamID 2, Size M = 35000) x 2 + Tiramisu (SanPhamID 7, Size S = 45000) x 1. Tổng: 115,000 đ + 5000 topping.
INSERT INTO chitietdonhang (ChiTietDonHangID, DonHangID, SanPhamSizeID, SoLuong, DonGia, ThanhTien, TrangThai) VALUES
(1, 1001, 1, 2, 25000.00, 50000.00, 'HoanThanh'), -- Cà phê đen size S
(2, 1001, 10, 1, 35000.00, 35000.00, 'HoanThanh'), -- Croissant
(3, 1002, 5, 2, 35000.00, 70000.00, 'HoanThanh'), -- Bạc xỉu size M
(4, 1002, 17, 1, 45000.00, 45000.00, 'HoanThanh') -- Tiramisu
ON CONFLICT (ChiTietDonHangID) DO NOTHING;

-- Trigger to auto-update NgayCapNhat on row update
...
CREATE OR REPLACE TRIGGER update_nguoidung_modtime
    BEFORE UPDATE ON nguoidung
    FOR EACH ROW
    EXECUTE FUNCTION update_modified_column();

CREATE OR REPLACE TRIGGER update_sanpham_modtime
    BEFORE UPDATE ON sanpham
    FOR EACH ROW
    EXECUTE FUNCTION update_modified_column();

CREATE OR REPLACE TRIGGER update_donhang_modtime
